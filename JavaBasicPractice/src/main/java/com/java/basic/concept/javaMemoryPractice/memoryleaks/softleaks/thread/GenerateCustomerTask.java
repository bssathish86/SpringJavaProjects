package com.java.basic.concept.javaMemoryPractice.memoryleaks.softleaks.thread;

import java.util.UUID;

public class GenerateCustomerTask implements Runnable {

	private CustomerManager cm;
	private int totalCustomersGenerated = 0;

	public GenerateCustomerTask(CustomerManager cm) {
		this.cm = cm;
	}

	@Override
	public void run() {
		while (true) {
			try {
				Thread.sleep((1000));
			} catch (InterruptedException e) {
				e.printStackTrace();
			}
			// Simulate user adding a customer through a web page
			String name = new UUID(1l, 10l).toString();
			Customer c = new Customer(name);
			cm.addCustomer(c);
			totalCustomersGenerated++;
			cm.getNextCustomer();
			System.out.println(
					Thread.currentThread().getName() + " TotalCustomersGenerated -> " + totalCustomersGenerated);
		}
	}

}
